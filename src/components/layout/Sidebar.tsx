...
import { LayoutDashboard, PanelLeftOpen, PanelLeftClose, Shield, LogOut } from "lucide-react"
    ...
    <>
      ...
      <aside
        className={cn(
          ...
          "md:relative md:w-60",
          mobileOpen ? "fixed inset-y-0 left-0 w-60" : "w-14",
        )}
      ...
      >
        ...
        <div
          className={cn(
            ...
            expanded ? "p-4 justify-between" : "p-2 flex-col gap-2 md:p-4 md:flex-row md:justify-between",
          )}
        >
          <button
            ...
            title="Abmelden"
            className={cn(
              "flex items-center gap-2 rounded-md text-sm text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-colors",
              expanded ? "px-3 py-2" : "p-2 md:px-3 md:py-2",
            )}
          >
            <LogOut className="h-4 w-4 shrink-0" />
            <span className={cn(expanded ? "inline" : "hidden", "md:inline")}>Abmelden</span>
          </button>
          <button
